...
module.exports = app => {
    ...
    const OrderFormSchema = new Schema({
        ...
        attachment: {type: String}, // 附件
        budget: {type: Number}, // 预算金额，null：未知（需面谈）
        deadline: {type: Date, required: true}, // 截止时间
        // 以上内容为用户填写内容
        // 以下内容为非用户填写内容（系统自动生成）
        createTime: {type: Date, default: () => new Date()}, // 订单发布时间，不可修改
        editedTime: {type: Date, default: () => new Date()}, // 订单最近发布时间or修改时间
        status: {type: Number, default: 0}, // 订单状态 0-已发布 1-已被接单 2-已完成 3-已取消
        publisher: {type: String, required: true}, // 发布人
